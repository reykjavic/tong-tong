...
- **Layout conventions:** MUI components + `sx` prop (or `styled`); `Container maxWidth="lg"` + responsive `{ xs, sm }` values; two shared layout wrappers: `PageLayout` (`src/components/layout/PageLayout.tsx`) provides the app-wide `main > Container > Paper` shell, and `PageContainer` (`src/components/layout/PageContainer.tsx`) wraps each page's content in the standard container + a screen-reader-only `<h1>` (pass `isMobile` to go full-bleed on small screens, as Contact does). Mobile-first via `useMediaQuery(theme.breakpoints.down('sm'))`.
- **Component structure:** `src/components/` is split into `layout/` (page chrome: Navbar, Footer, PageLayout), `ui/` (dumb presentational primitives: ContentCard, VisuallyHiddenH1, typography), and `features/` (product-specific units: OpeningHours, PageMeta, Markdown). A `ui/` component has no hooks or logic — it renders children with a style.
