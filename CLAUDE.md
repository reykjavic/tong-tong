...
- **Data layer:** server state via **TanStack Query** (`useQuery`/`useMutation`; `QueryClientProvider` wraps both entry points via `src/hooks/queryClient.ts`); client state via **Zustand** (auth session in `auth.ts`; the future cart). **`src/hooks/api.ts` is the single file for every request** — URLs, query keys, `apiFetch`, all endpoints and hooks (config/hours/orders/toggle/auth/GitHub); server-state consumers keep a small wrapper (e.g. `useConfig()` → `{ status, config }`) so pages don't touch query details.
