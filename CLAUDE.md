...
- **Entry flow:** `src/main.tsx` → `src/App.tsx`, which wraps everything in `ThemeProvider` (MUI) + `I18nProvider`, then renders `Navbar`, a `Switch` of Wouter routes, and `Footer`. All routes and their components are registered in `App.tsx`.
- **Routing:** Wouter (`<Route path="/" component={...}>` / `<Link to="/...">`). Routes: `/`, `/about`, `/menu`, `/buffet`, `/contact`, `/posts`, `/impressum`, `/datenschutz`. Several pages are placeholders (`About.tsx`, `Contact.tsx`, `Menu.tsx` render a bare `<h1>`; others render placeholder German text) — the fully built page is `HomePage.tsx`.
...
## Decap CMS (planned, not yet implemented)

`SPEC.md` specifies a Decap CMS for managing news posts as Markdown in `content/posts/` served from `public/admin/`. Neither directory exists yet — posts/feed features are currently stubbed. `public/` is otherwise empty.

## Deployment

GitHub Actions workflow `.github/workflows/deploy.yml` deploys on push/merge to `main`: `npm ci` → `npm run build` → `aws s3 sync dist/ s3://tong-tong-homepage --delete` (region `eu-central-1`). Live site is the S3 static-hosting endpoint (HTTP only). Known issues in `TODO.md`: SPA deep links like `/menu` 404 on direct visit (no error-document → `index.html` configured yet), and no CloudFront/HTTPS/invalidation yet.
