...
- **Layout conventions:** MUI components + `sx` prop (or `styled`); `Container maxWidth="lg"` + responsive `{ xs, sm }` values; the shared `PageLayout` component (`src/components/layout/PageLayout.tsx`) wraps every route in a `main > Container > Paper` shell. Mobile-first via `useMediaQuery(theme.breakpoints.down('sm'))`.
