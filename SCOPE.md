...
| GET    | `/api/hours`                   | public      | hours   | Real opening hours from the Google Business Profile (Places API, cached 24h) — closure state + the special/vacation-adjusted next-7-days view (the regular week matches the site's hardcoded schedule and is not fetched); drives the site's open/closed chip and the dashboard's order-polling gate |
